...
const app = {
        ...
        Swal.fire({
            ...
            html: `
                <div class="text-left space-y-3.5 my-1 text-sm">
                    ...
                    <div class="p-3 rounded-xl bg-violet-50/80 dark:bg-violet-950/40 border border-violet-100 dark:border-violet-900/50 text-xs text-violet-900 dark:text-violet-200 space-y-1">
    ...
    promptPwaInstall: async () => {
        if (!window.pwaInstallPrompt && !deferredPrompt) {
            Swal.fire({
                icon: 'info',
                title: 'Install Krishan POS App',
                html: `
                    <div class="text-left text-sm space-y-2">
                        ...
                        <ul class="list-disc pl-5 space-y-1 text-xs text-slate-600 dark:text-slate-300">
                            <li><b>On Android Chrome:</b> Tap the 3 dots (⋮) menu and choose <b>"Install app"</b> or <b>"Add to Home screen"</b>.</li>
                            <li><b>On iPhone Safari:</b> Tap the Share button (⎋) and choose <b>"Add to Home Screen"</b>.</li>
                            <li><b>On Windows Edge/Chrome:</b> Click the Install icon (⊞) in the browser address bar.</li>
                        </ul>
                    </div>
                `,
                confirmButtonColor: '#7c3aed'
        ...
        const promptEvent = window.pwaInstallPrompt || deferredPrompt;
        if (promptEvent) {
            promptEvent.prompt();
        ...
        if (container.classList.contains('hidden')) {
            ...
            try {
                if (typeof Html5QrcodeScanner !== 'undefined') {
                    app.state.modalScanner = new Html5QrcodeScanner("modal-reader", {
                        fps: 10,
                        qrbox: { width: 220, height: 150 },
                        aspectRatio: 1.0
                    }, false);

                    app.state.modalScanner.render((decodedText) => {
                        const input = document.getElementById('swal-barcode');
                        if (input) {
                            input.value = decodedText;
                            app.updateBarcodePreview(decodedText);
                        }
                        app.toggleItemScanner();
                    }, (err) => {
                        // ignore frame parse errors
                    });
                }
            } catch (e) {
                console.warn('Item scanner init error:', e);
            }
        } else {
            container.classList.add('hidden');
            if (app.state.modalScanner) {
                try {
                    app.state.modalScanner.clear();
                } catch (e) {}
                app.state.modalScanner = null;
            }
        }
    },
    ...
    renderRepairs: async () => {
        ...
        if (searchQuery) {
            filteredRepairs = filteredRepairs.filter(r => {
                ...
                const model = (r.phoneModel || '').toLowerCase();
        ...
        const html = `
            <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-4 sm:p-6 fade-in h-full flex flex-col space-y-5">
                <!-- Top Header & Action -->
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    ...
                    <button onclick="app.openRepairModal()" class="w-full sm:w-auto bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-violet-500/25 transition-all flex items-center justify-center gap-2 text-sm active:scale-95">
                    ...
                    ` : `
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-4">
                            ${filteredRepairs.map(job => {
                                const estCost = Number(job.estimatedCost !== undefined ? job.estimatedCost : (job.cost || 0));
                                const advPay = Number(job.advancePayment || 0);
                                ...
                                <div class="border border-slate-200/80 dark:border-slate-700/80 rounded-2xl p-4 hover:shadow-lg transition-all bg-white dark:bg-slate-800/90 relative flex flex-col justify-between space-y-3">
                                    ...
                                    <div class="flex justify-between items-start">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-[10px] font-bold px-2 py-0.5 rounded-full ${app.getStatusColor(job.status)}">
                                                ${job.status || 'Pending'}
                                            </span>
                                        </div>
                                    ...
                                    <div>
                                        <h3 class="font-black text-slate-800 dark:text-white text-base leading-snug flex items-center gap-1.5">
                                            <i class="fa-solid fa-mobile-screen text-violet-600 text-sm"></i> ${job.phoneModel}
                                        </h3>
                                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 dark:text-slate-400 mt-1">
                                            <span class="font-semibold"><i class="fa-solid fa-user mr-1 text-slate-400"></i>${job.customerName || 'Walk-in'}</span>
                                            ${job.contact ? `
                                                <a href="tel:${job.contact}" class="font-bold text-violet-600 dark:text-violet-400 hover:underline">
                                                    <i class="fa-solid fa-phone mr-1 text-emerald-500"></i>${job.contact}
                                                </a>
                                    ...
                                    <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-700 text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
                                        ...
                                        ${job.issue || 'General Service / Checkup'}
                                    </div>
                                    ...
                                    <div class="pt-2 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between gap-1.5">
                                        ...
                                        <button onclick="app.printRepairReceipt(${job.id})"
                                            ...
                                            title="Print Customer Repair Bill / Job Slip">
                                            <i class="fa-solid fa-print"></i>
                                            <span>Bill Print</span>
                                        </button>

                                        <button onclick="app.updateRepairStatus(${job.id})"
                                            class="py-2 px-2.5 rounded-xl bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 text-slate-700 dark:text-slate-200 text-xs font-bold transition flex items-center gap-1"
                                        ...
                                        <button onclick="app.openRepairModal(${job.id})"
                                            class="py-2 px-2.5 rounded-xl bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 text-slate-700 dark:text-slate-200 text-xs font-bold transition"
                                            title="Edit Job Details">
                                            <i class="fa-solid fa-pen"></i>
                                        </button>

                                        <button onclick="app.deleteRepair(${job.id})"
                                            class="py-2 px-2.5 rounded-xl bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 text-rose-600 text-xs font-bold transition"
                                            title="Delete Job">
                                            <i class="fa-solid fa-trash"></i>
                                        </button>
                                    </div>
    ...
    openRepairModal: async (id = null) => {
        ...
        const { value: formValues } = await Swal.fire({
            ...
            html: `
                <div class="space-y-3 text-left text-xs my-2">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                            <label class="block font-bold text-slate-600 dark:text-slate-400 mb-1">Customer Phone (දුරකථන අංකය)</label>
                            <input id="rep-contact" type="tel" class="swal2-input !m-0 !w-full !text-xs font-semibold" placeholder="07x xxxxxxx" value="${job.contact || ''}">
                        </div>
                    </div>
            ...
            preConfirm: () => {
                ...
                if (!customerName || !phoneModel || !issue) {
                    Swal.showValidationMessage('Please fill in Customer Name, Device Model, and Issue Description');
                    return false;
                }
        ...
        if (formValues) {
            ...
            // Ask to print customer bill / job card slip
            const printAsk = await Swal.fire({
                title: id ? 'Print Customer Job Bill?' : 'Repair Job Registered!',
                html: `
                    <div class="text-center space-y-2 text-sm">
                        <div class="font-mono text-xl font-black text-violet-700 dark:text-violet-400">
                            #REP-${String(targetId).padStart(4, '0')}
                        </div>
                        <p class="text-xs text-slate-500">Would you like to print the official Repair Job Slip for the customer?</p>
                        <p class="text-xs font-bold text-slate-700 dark:text-slate-300">ගනුදෙනුකරුට රෙපයාර් බිල්පත (Job Card) Print කරන්නද?</p>
                    </div>
                `,
                icon: 'question',
                showCancelButton: true,
                confirmButtonText: '<i class="fa-solid fa-print mr-1.5"></i> Yes, Print Bill (බිල්පත Print කරන්න)',
                cancelButtonText: 'Done (අවශ්‍ය නැත)',
                confirmButtonColor: '#7c3aed'
            });

            if (printAsk.isConfirmed) {
                app.printRepairReceipt(targetId);
            }
        }
    },
    ...
    updateRepairStatus: async (id) => {
        ...
        if (status) {
            ...
            // Prompt delivery print if delivered
            if (status === 'Delivered' || status === 'Completed') {
                const printAsk = await Swal.fire({
                    title: `Status set to ${status}`,
                    text: 'Would you like to print the updated repair receipt?',
                    icon: 'info',
                    showCancelButton: true,
    ...
    // ──────────────────────────────────────────────
    // REPAIR BILL & JOB CARD THERMAL PRINTING
    // ──────────────────────────────────────────────
    printRepairReceipt: async (repairId) => {
        const job = await db.repairs.get(repairId);
        if (!job) {
            Swal.fire('Error', 'Repair job not found', 'error');
            return;
        }
        ...
        const shopDetails = {
            ...
            phone: localStorage.getItem('krishan_pos_shop_phone') || "076 928 1880 / 071 759 7335",
            email: localStorage.getItem('krishan_pos_shop_email') || "krishanpos@gmail.com"
        };

        const tokenNo = `#REP-${String(job.id).padStart(4, '0')}`;
        const barcodeVal = `REP-${String(job.id).padStart(4, '0')}`;
        const estCost = Number(job.estimatedCost !== undefined ? job.estimatedCost : (job.cost || 0));
        const advPay = Number(job.advancePayment || 0);
        const balance = Math.max(0, estCost - advPay);
        const formattedDate = new Date(job.createdAt || Date.now()).toLocaleString('en-US', {
        ...
        const receiptHTML = `
            ...
            <head>
                <meta charset="UTF-8">
                <title>Repair Job ${tokenNo}</title>
                <style>
                    * { box-sizing: border-box; margin: 0; padding: 0; }
                    ...
                    @media print {
                        body { margin: 0; padding: 4px 6px; width: 100%; }
                        @page { margin: 0; size: auto; }
                    }
                    .header { text-align: center; margin-bottom: 5px; }
                    .shop-name { font-size: 16px; font-weight: 900; text-transform: uppercase; letter-spacing: -0.3px; line-height: 1.1; }
                    .shop-sub { font-size: 11px; font-weight: 700; color: #222; margin-top: 2px; }
                    .shop-address { font-size: 10px; font-weight: 500; color: #444; margin-top: 1px; }
                    .shop-contact { font-size: 11px; font-weight: 800; margin-top: 3px; }
                    ...
                    .token-title { font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; }
                    .token-num { font-size: 20px; font-weight: 900; letter-spacing: 1px; font-family: monospace; line-height: 1.1; }
                    .token-status { font-size: 10px; font-weight: 700; text-transform: uppercase; margin-top: 2px; }
                    .row { display: flex; justify-content: space-between; margin-bottom: 2.5px; font-size: 12px; }
                    ...
                    .barcode-container { text-align: center; margin: 6px 0 3px 0; }
                    .barcode-container svg { max-width: 100%; height: 32px; }
                    .sign-box { display: flex; justify-content: space-between; margin-top: 24px; font-size: 9.5px; font-weight: 600; }
                    .sign-line { border-top: 1px dotted #000; width: 44%; text-align: center; padding-top: 2px; }
                </style>
            </head>
            <body>
                <!-- SHOP HEADER -->
                <div class="header">
                    ...
                    <div class="shop-sub">${shopDetails.sub}</div>
                    <div class="shop-address">${shopDetails.address}</div>
                    <div class="shop-contact">📞 ${shopDetails.phone}</div>
                </div>
                ...
                <!-- JOB TOKEN BOX -->
                <div class="token-box">
                    <div class="token-title">REPAIR JOB CARD / රෙපයාර් බිල්පත</div>
                    <div class="token-num">${tokenNo}</div>
                    <div class="token-status">Status: ${(job.status || 'PENDING').toUpperCase()}</div>
                </div>
                ...
                <div style="margin: 4px 0;">
                    <div class="row">
                        ...
                        <span class="val">${job.customerName || 'Walk-in'}</span>
                    </div>
                    <div class="row">
                        <span class="label">Customer Phone:</span>
                        <span class="val">${job.contact || 'N/A'}</span>
                    </div>
                    <div class="row" style="margin-top: 3px;">
                        <span class="label">Device / Model:</span>
                        <span class="val" style="font-size: 14px; font-weight: 900;">${job.phoneModel}</span>
                    </div>
                </div>

                <!-- FAULT / ISSUE -->
                <div class="box">
                    <div style="font-size: 9.5px; font-weight: 800; text-transform: uppercase; color: #444;">Fault / දෝෂය:</div>
                    <div style="font-weight: 700; font-size: 12px; margin-top: 1px;">${job.issue || 'General Inspection / Repair'}</div>
                </div>
                ...
                <div style="margin: 4px 0;">
                    <div class="row">
                        <span class="label">Estimated Cost (ඇස්තමේන්තුව):</span>
                        <span class="val">LKR ${estCost.toFixed(2)}</span>
                    </div>
                    <div class="row">
                        <span class="label">Advance Paid (අත්තිකාරම්):</span>
                        <span class="val" style="color: #059669;">LKR ${advPay.toFixed(2)}</span>
                    </div>
                    <div class="balance-box">
                        <span>Balance Due (ඉතිරි මුදල):</span>
                ...
                <!-- TERMS & CONDITIONS -->
                <div class="terms">
                    <p style="font-weight: 800; margin-bottom: 1.5px;">* කරුණාකර භාණ්ඩය ආපසු ලබා ගැනීමට මෙම බිල්පත ඉදිරිපත් කරන්න.</p>
                    <p style="font-weight: 600;">* Please bring this receipt when collecting your device.</p>
                    <p>* දින 30 කට පසු රැගෙන නොයන භාණ්ඩ පිළිබඳ වගකීමක් නොදරනු ලැබේ.</p>
                    <p style="margin-top: 4px; font-weight: 900; font-size: 11px;">Thank You! Krishan Communication</p>
                </div>
                    ...
                    window.onload = function() {
                        try {
                            if (typeof JsBarcode !== 'undefined') {
                                JsBarcode("#repair-barcode", "${barcodeVal}", {
                                    format: "CODE128",
                                    width: 1.5,
                                    height: 30,
                                    displayValue: true,
